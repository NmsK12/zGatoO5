            ...
            for msg in messages:
                if msg.date.timestamp() > current_timestamp - 120:  # Últimos 2 minutos
                    ...
                    if is_from_bot and msg.text:
                        ...
                        if "[✖️] No se encontro informacion para los datos ingresados." in msg.text:
                            logger.info(f"[{request_id}] Mensaje de 'No se encontró información' detectado para NM {nombres}|{apellidos}")
                            return {
                                'success': False,
